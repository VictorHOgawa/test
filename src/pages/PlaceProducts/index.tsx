...
export default function PlaceProducts() {
  ...
  async function getProducts() {
    const connect = await getAPI(`/place/${productId}/product/current`);
  ...
  const filteredProducts = type ? product.filter((item: any) => item.category === type) : product;
      ...
      <View className="w-full p-2 absolute bottom-5 z-20 justify-between mb-0 flex flex-col ">
        <View className=" flex w-full justify-between flex-row ">
          <TouchableOpacity
            ...
            className=" w-44 h-14 bg-[#6B1DA5]/90 relative overflow-hidden border-[#6B1DA5] border-2 rounded-2xl flex flex-row items-center justify-center"
          >
          ...
          <TouchableOpacity
            onPress={() => handleGoToCheckout()}
            className=" w-44 h-14 bg-[#447f34]/95 border-2 border-[#75FB4C] rounded-2xl flex flex-row items-center justify-center"
          >
            <Image
              className="w-7 h-7  left-2 absolute mr-1"
              source={require('../../../assets/Global/Icons/finishEventProducts.png')}
            />
            <Text className="text-[18px] ml-2 self-center text-white font-poppinsBold">
              Finalizar
            </Text>
          </TouchableOpacity>
        </View>
        <TouchableOpacity className="w-[80vw] bg-[#b5afaf]/90 border-[1px] border-white self-center rounded-xl h-10 mt-2 flex flex-row items-center p-2">
          ...
          />
          <Text className="text-white text-[11px] font-poppinsBold">
            {' '}
              ...
              {filteredProducts.map((item: any) => (
                ...
                >
                  ...
                  <View className="flex flex-col justify-center ml-3">
                    <Text className="text-[14px] font-poppinsBold text-white w-[135px]">
                      {item.name}
                    </Text>
                    <Text className="text-[14px] self-start font-poppinsRegular font-light text-white">
                  ...
                  <View className="flex flex-row items-center">
                    ...
                    <TouchableOpacity onPress={() => handleChange('increase', item)}>
                      {productQuantity(item.id) === 0 ? (
                        <Image
